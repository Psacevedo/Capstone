...
\textbf{Simulaci\'on Monte Carlo (P4).} Cada cliente inicia con $C_0 = 1\,000$ USD. La utilidad de FinPUC proviene exclusivamente de una comisi\'on mensual de 0,5\% sobre el saldo activo administrado, acumulada en la implementaci\'on semanal con tasa equivalente $k_{\text{sem}} = 0{,}005 \times 12/52$; no hay comisiones por transacci\'on ni por rebalanceo. El comportamiento del cliente combina una probabilidad de aceptaci\'on de la recomendaci\'on y una probabilidad de abandono semanal. La aceptaci\'on se modela como $p_{\text{act}}=1/(1+\exp[-(R_{\text{proyectado}}-\hat{x})])$, donde $R_{\text{proyectado}}$ es el retorno anual ofertado y $\hat{x}$ la tolerancia de p\'erdida del perfil. El abandono semanal usa el modelo \texttt{views\_v1\_plus}: la p\'erdida se mide respecto de $C_0$ y la probabilidad conductual sigue una log\'istica sin reescalar, $1/(1+\exp(-(x_t-\hat{x})))$, activa solo cuando la p\'erdida supera la tolerancia $\hat{x}$; adem\'as, si la riqueza llega a cero, el abandono es forzoso (restricci\'on de ruina). Para el perfil muy conservador, con tolerancia 0\%, cualquier p\'erdida genera probabilidad de abandono positiva. Se ejecutan 2\,000 trayectorias por combinaci\'on, cantidad considerada adecuada para estabilizar riqueza terminal, tasa de abandono y utilidad de la empresa.
...
Aceptaci\'on de recomendaci\'on & $p_{\text{act}}=1/(1+\exp[-(R_{\text{proyectado}}-\hat{x})])$ \\
