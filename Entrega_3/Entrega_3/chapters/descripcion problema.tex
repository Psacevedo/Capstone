...
\subsection{Problema de negocio: comisi\'on \'optima y balance cliente-empresa}\label{sec:problema_negocio}

La Entrega 2 del proyecto fij\'o la comisi\'on en $K = 1\%$ sobre el monto
transado y evalu\'o las alternativas mediante un Score P4 que suma linealmente
riqueza del cliente y utilidad de la empresa, ponderando ambas con igual peso.
Esta formulaci\'on deja dos preguntas abiertas que la presente fase aborda.

En primer lugar, la tasa de comisi\'on $K\%$ determina simult\'aneamente la
utilidad de FinPUC y la riqueza neta del cliente. Un $K$ alto genera m\'as
ingresos por transacci\'on, pero reduce el capital invertido y puede elevar la
tasa de retiro si el cliente percibe que la plataforma es demasiado costosa.
Un $K$ bajo favorece al cliente pero compromete la sostenibilidad financiera
de la plataforma. La pregunta central es: ¿cu\'al es el valor de $K$ que
maximiza el Score P4, balanceando riqueza del cliente, utilidad de la empresa y
retenci\'on?

En segundo lugar, el Score P4 original asigna el mismo peso a la riqueza del
cliente y a la utilidad de la empresa. En la pr\'actica, FinPUC podr\'ia
priorizar uno u otro objetivo seg\'un su estrategia de negocio. Para
formalizar esta decisi\'on, se introduce un par\'ametro de balance
$\lambda \in [0,1]$ que pondera:
\begin{equation}
\text{Score}_{\lambda} = \lambda \cdot f_1(w) + (1-\lambda) \cdot f_2(w),
\end{equation}
donde $f_1(w)$ es el retorno esperado del cliente (normalizado) y $f_2(w)$ es
la utilidad esperada de FinPUC (normalizada). Con $\lambda = 1$, la plataforma
optimiza exclusivamente el retorno del cliente; con $\lambda = 0$, solo su
propia utilidad. La pregunta es: ¿qu\'e valor de $\lambda$ entrega el mejor
desempe\~no conjunto seg\'un el Score P4?

La respuesta a ambas preguntas requiere (a) calibrar el modelo Black-Litterman
para que las proyecciones de Monte Carlo reflejen la mejor configuraci\'on
posible de hiperpar\'ametros, (b) evaluar la sensibilidad del Score P4 a $K$ y
$\lambda$ bajo esa configuraci\'on, y (c) comparar distintas
\textit{views} y modelos de abandono para asegurar que la recomendaci\'on final
es robusta a la elecci\'on de estos componentes.
