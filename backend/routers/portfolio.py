"""
portfolio.py - Endpoints del sistema recomendador FinPUC (optimización académica).
"""
import json
import logging
import sqlite3
...
from ..services.methodology_catalog import get_base_parameters, get_catalog, get_methodology
from ..services.universe_f5 import F5_MIN_HISTORY_ROWS, F5_MIN_MARKET_CAP, F5_MIN_PRICE, f5_base_clauses, f5_base_where_sql
from ..services.return_estimators import estimate_returns
from ..utils.candidate_selection import (
    align_return_matrix,
    build_benchmark_portfolio,
    fetch_price_series,
    select_candidates,
    select_market_cap_candidates,
)
from ..utils.frontier_analysis import (
    build_frontier_markers,
    compute_efficient_frontier_points,
    frontier_metric_point,
)
...
from ..utils.hybrid_solver_utils import run_hybrid_solver
from ..utils.payloads import (
    ...
    portfolio_rows,
    scenarios_payload,
    scenarios_timeseries_payload,
    simulation_payload,
    validation_summary,
)
from ..utils.portfolio_resolver import (
    LEGACY_METHOD_MAP,
    RISK_PROFILES,
    coerce_query_default,
    ensure_required_inputs,
    normalize_parameter_values,
    optimizer_universe_size,
    resolve_candidate_pool_size,
    resolve_methodology_id,
    resolve_profile,
    resolve_target_holdings,
    risk_level,
)
...
def _fit_distributions_pct(values_pct: np.ndarray) -> Dict:
    """Ajusta distribuciones candidatas sobre retornos en % para diagnostico."""
    ...
    if sigma > 0:
        ...
        candidates.append({
            ...
            "loglik": round(ll, 3), "aic": round(aic, 3), "bic": round(bic, 3),
        })
    ...
    try:
        ...
        if float(scale) > 0:
            ...
            candidates.append({
                "name": "johnsonsu",
                "params": {"a": round(float(a), 6), "b": round(float(b), 6), "loc_pct": round(float(loc), 6), "scale_pct": round(float(scale), 6)},
                "loglik": round(ll, 3), "aic": round(aic, 3), "bic": round(bic, 3),
            })
    except Exception:
        pass
    ...
    notes = ("Guia rapida: kurtosis_excess alta => colas pesadas (Student-t). Skew != 0 => asimetria.")
    if implied_df is not None and np.isfinite(implied_df):
        notes += f" df(t) aproximado: {round(float(implied_df), 3)}."
...
class SimulateRequest(BaseModel):
    ...
    n_simulations: int = Field(default=5000, ge=500, le=20000)
    commission_rate_pct: float = Field(default=1.0, ge=0, le=10)
    p2_acceptance_prob_pct: Optional[float] = Field(default=None, ge=0, le=100)
    rebalance_freq_weeks: int = Field(default=1, ge=1, le=52)
    rebalance_return_boost_pct: float = Field(default=0.0, ge=0, le=5)
...
def resolve_methodology_id(req: OptimizeRequest) -> str:
...
def resolve_profile(req: OptimizeRequest) -> Tuple[str, Dict, float]:
    ...
    profile_key = risk_level(max_loss_pct)
    profile_cfg = RISK_PROFILES[profile_key]
    return profile_key, profile_cfg, profile_cfg["alpha_p"]
...
def frontier_metric_point(
    ...
    point: Dict[str, object] = portfolio_metrics(weights_arr, returns_matrix, ann_returns, risk_free_rate)
    point["label"] = label
...
def _candidate_query(profile_cfg: Dict, sector_filter: Optional[str]) -> Tuple[str, List, str]:
    ...
    # El vol cap se aplica como restriccion de optimizacion (con relajacion),
    # NO como filtro de candidatos (muy restrictivo con datos reales).
    # Ver _run_primary_optimizer para la relajacion automatica.
...
def build_benchmark_portfolio(candidates: List[Dict], target_holdings: int) -> Tuple[List[str], np.ndarray]:
...
def parse_views_json(raw_views: object, tickers: Sequence[str]) -> Tuple[np.ndarray, np.ndarray, List[Dict]]:
...
def _black_litterman_ann_returns(
    ...
    ann_cov = np.cov(returns_matrix.T) * 252 if returns_matrix.shape[1] > 1 else np.array([[returns_matrix.var() * 252]])
    if ann_cov.shape[0] > 1:
        ann_cov = 0.80 * ann_cov + 0.20 * np.diag(np.diag(ann_cov))
    ann_cov += np.eye(len(tickers)) * 1e-8

    # Prior de equilibrio: pi = delta * Sigma * w_mkt
    market_caps = np.array(
        [max(float(metadata.get(ticker, {}).get("market_cap") or 1.0), 1.0) for ticker in tickers],
        dtype=float,
    )
    ...
    # Construir views: predefinidas (view_type) o manuales (views_json)
    view_type = parameter_values.get("bl_view_type")
    views_json = parameter_values.get("views_json")

    if view_type and str(view_type) in VIEW_CONFIGS:
        p_matrix, q_vector, confidence, view_info = build_bl_view(
            str(view_type), list(tickers), returns_matrix, metadata,
        )
        omega = np.atleast_2d(
            (p_matrix @ (tau * ann_cov) @ p_matrix.T) / max(confidence, 1e-8)
        )
        omega = np.maximum(omega, 1e-12)
        views_used = [view_info]
    elif views_json:
        p_matrix, q_vector, views_used = parse_views_json(views_json, tickers)
        default_conf = _normalize_numeric(parameter_values.get("bl_confidence"), 0.50) or 0.50
        omega = np.atleast_2d(
            (p_matrix @ (tau * ann_cov) @ p_matrix.T) / max(default_conf, 1e-8)
        )
        omega = np.maximum(omega, 1e-12)
    else:
        raise HTTPException(
            status_code=400,
            detail="Debes seleccionar un tipo de view predefinido (bl_view_type) o ingresar views manuales (views_json).",
        )

    # Posterior BL: mu_bl = prior + tau*Sigma*P' * inv(P*tau*Sigma*P' + Omega) * (Q - P*prior)
    tau_sigma = tau * ann_cov
    middle = p_matrix @ tau_sigma @ p_matrix.T + omega
    try:
        middle_inv = np.linalg.inv(middle)
    except np.linalg.LinAlgError:
        middle_inv = np.linalg.pinv(middle)
    adjustment = tau_sigma @ p_matrix.T @ middle_inv @ (q_vector - p_matrix @ prior)
    mu_bl = prior + adjustment
    ...
    return np.array(mu_bl, dtype=float), {
        ...
        "view_type": str(view_type) if view_type else "manual",
...
def validation_summary(
    ...
    price_map = closes_from_series(fetch_price_series(db, tickers, splits["validation_start"], splits["validation_end"]))
    contributions = []
    for ticker, weight in zip(tickers, weights):
...
def _run_primary_optimizer(
    ...
    max_vol = (profile_cfg or {}).get("max_vol") if profile_cfg else None
    ...
    if methodology_id == "equiponderado":
        ...
        # Intentar con vol cap; si no converge, relajar (Entrega 2)
        ...
        if not optimized.get("weights") and max_vol is not None:
            log.warning("Markowitz no convergio con vol cap=%.2f, relajando restriccion.", max_vol)
            optimized = compute_markowitz_portfolio(
                list(tickers), returns_matrix, risk_free_rate,
                custom_ann_returns=ann_returns,
                profile=profile, max_weight=max_weight, gamma=gamma,
                max_vol=None,
            )
...
def _benchmark_payload(
    ...
    """Benchmark academico: top-N por market cap, pesos iguales (Entrega 2)."""
    target_holdings = target_holdings or 20
    # Seleccionar top por market cap (no por CAGR)
    candidates, total_f5_count = select_market_cap_candidates(db, max(100, target_holdings), sector)
    selected = candidates[:target_holdings]
    if not selected:
    ...
    series = fetch_price_series(db, selected_tickers, splits["calibration_start"], splits["calibration_end"])
    close_map = closes_from_series(series)
    min_len = min(len(close_map[ticker]) for ticker in selected_tickers)
    returns_matrix = np.column_stack([daily_returns(close_map[ticker][-min_len:]) for ticker in selected_tickers])
    ann_returns = _historical_ann_returns(selected_tickers, metadata, returns_matrix)
    metrics = portfolio_metrics(selected_weights, returns_matrix, ann_returns, DEFAULT_RISK_FREE_RATE)
    metrics["method"] = "benchmark"
    metrics["method_label"] = f"Benchmark top-{len(selected_tickers)} market cap"
    ...
    return {
        ...
        "universe": {
            ...
            "target_holdings": len(selected_tickers),
...
def _base_case_payload(
    ...
    candidates, total_f5_count = select_market_cap_candidates(db, max(200, n_holdings), sector)
    if not candidates:
    ...
    splits = get_split_dates()
    series = fetch_price_series(db, requested_tickers, splits["calibration_start"], splits["calibration_end"])
    selected_tickers, returns_matrix = align_return_matrix(series, requested_tickers)
    selected_weights = np.ones(len(selected_tickers), dtype=float) / len(selected_tickers)
    ann_returns = _historical_ann_returns(selected_tickers, metadata, returns_matrix)
    metrics = portfolio_metrics(selected_weights, returns_matrix, ann_returns, DEFAULT_RISK_FREE_RATE)
    cvar_level = 0.90
    ...
    validation = validation_summary(db, selected_tickers, selected_weights, splits)
    rebalance_policy = {
        "summary": f"Top {len(selected_tickers)} por capitalizacion de mercado, pesos iguales y rebalanceo mensual.",
        "selection_criterion": "Empresas de mayor capitalizacion de mercado del universo de acciones.",
    ...
    return {
        ...
        "methodology": {
            ...
            "description": "Punto de comparacion obligatorio: empresas de mayor capitalizacion del universo de acciones, pesos iguales, rebalanceo mensual y simulacion del cliente.",
        ...
        "weekly_cycle": build_weekly_cycle(
            {
        ...
        "universe": {
            ...
            "sector_filter": sector,
            "selection": "Orden descendente por market_cap dentro del universo de acciones.",
        },
    }
...
def optimize_portfolio(
    ...
    methodology_id = resolve_methodology_id(req)
    if methodology_id == "benchmark":
        payload = _benchmark_payload(
            db=db,
            target_holdings=resolve_target_holdings(req),
    ...
    splits = get_split_dates()
    target_holdings = resolve_target_holdings(req)
    profile_key, profile_cfg, alpha_p = resolve_profile(req)

    prelim_candidates, total_f5_count = select_candidates(
        db=db,
        profile_cfg=profile_cfg,
        candidate_pool_size=636,
        sector_filter=req.sector,
    )
    candidate_pool_size = resolve_candidate_pool_size(profile_cfg, req.candidate_pool_size, total_f5_count)
    if methodology_id == "equiponderado":
    ...
    if not candidates:
        raise HTTPException(status_code=404, detail="No se encontraron acciones que cumplan los criterios del universo de acciones.")

    metadata = {candidate["ticker"]: candidate for candidate in candidates}
    opt_size = optimizer_universe_size(len(candidates), target_holdings)
    optimizer_candidates = candidates[:opt_size]
    optimizer_tickers = [candidate["ticker"] for candidate in optimizer_candidates]

    calibration_series = fetch_price_series(
        db,
        optimizer_tickers,
    ...
    methodology_params = normalize_parameter_values(methodology, req.parameter_values)
    ensure_required_inputs(methodology, methodology_params)
    ...
    if methodology_id == "finpuc_hibrido":
        optimized_weights, cash_weight, solver_details = run_hybrid_solver(
            valid_tickers,
            metadata,
            returns_matrix,
            ann_returns,
        ...
        selected_tickers, selected_weights, selected_returns, selected_ann_returns = trim_portfolio(
            valid_tickers,
            optimized_weights,
            returns_matrix,
            ann_returns,
    ...
    parameters_used = build_parameter_groups(
        methodology,
        methodology_params,
        profile_key,
        profile_cfg,
    ...
    if methodology_id != "finpuc_hibrido":
        ...
        frontier_markers = build_frontier_markers(
            valid_tickers,
            returns_matrix,
            ann_returns,
            risk_free_rate,
    ...
    response = {
        ...
        "weekly_cycle": build_weekly_cycle(methodology_params),
        ...
        "supuestos": {
            "aplicados": [
                {"label": "Tasa libre de riesgo (Rf)", "value": "2% anual", "reference": "Supuestos F1 / Seccion 4"},
                {"label": "Shrinkage covarianza (lambda)", "value": "0.20", "reference": "Supuestos E1 / Metodologia"},
                {"label": "Comision (k)", "value": f"{round(commission_rate_pct, 2)}% anual", "reference": "Supuestos F2 / Seccion 2.4"},
                {"label": "Gamma del perfil", "value": str(profile_cfg.get("gamma", "—")), "reference": "Supuestos D1-D5 / Informe 1"},
                {"label": "Peso maximo por activo", "value": f"{round((profile_cfg.get('max_weight', 0) or 0) * 100)}%", "reference": "Supuestos C5 / Seccion 2.4"},
                {"label": "Cota volatilidad", "value": f"{round((profile_cfg.get('max_vol', 0) or 0) * 100)}% anual" if profile_cfg.get("max_vol") else "Sin cota", "reference": "Supuestos D1-D5 / Seccion 2.3"},
                {"label": "Rebalanceo optimizacion", "value": "Semestral (126 dias)", "reference": "Supuestos J1 / Seccion 2.2"},
                {"label": "Dias habiles por ano", "value": "252", "reference": "Supuestos E3"},
                {"label": "Solo largo (long-only)", "value": "w_i >= 0", "reference": "Supuestos C1 / Seccion 2.4"},
                {"label": "Presupuesto completo", "value": "sum(w_i) = 1", "reference": "Supuestos C2 / Seccion 2.4"},
                {"label": "Solver", "value": "SLSQP (scipy.optimize)", "reference": "Supuestos G1 / Implementacion"},
            ],
            "simulacion": [
                {"label": "Capital inicial (C0)", "value": "$1,000 USD", "reference": "Supuestos K1 / P4"},
                {"label": "Sensibilidad logistica (s)", "value": "20", "reference": "Supuestos K4 / Ecuacion 4.6"},
                {"label": "Turnover por aceptacion", "value": "5% de la riqueza", "reference": "Supuestos K7 / Seccion 2.4"},
                {"label": "Simulaciones", "value": "5,000 trayectorias", "reference": "Supuestos K8 / P4"},
                {"label": "Horizonte", "value": "5 anos (260 semanas)", "reference": "Supuestos K2 / P4"},
                {"label": "Rebalanceo simulacion", "value": "Semanal", "reference": "Supuestos K3 / P4"},
                {"label": "Escenarios proyectados", "value": "+/- 0.5 * sigma", "reference": "Supuestos L1-L4 / P4"},
                {"label": "Medicion de perdida (P1)", "value": "Respecto a C0", "reference": "Supuestos K6 / P4"},
            ],
        },
    }
...
def get_benchmark(
    ...
    target_holdings = coerce_query_default(target_holdings)
    candidate_pool_size = coerce_query_default(candidate_pool_size)
    sector = coerce_query_default(sector)
    n = coerce_query_default(n)
...
def get_return_diagnostics(
    ...
    """
    Diagnostico rapido de distribuciones de retornos (normalidad) sobre el universo de acciones filtrado.
    Por defecto toma los top `n` por market cap (representativo para presentacion).
    """
    n = int(coerce_query_default(n))
    order = str(coerce_query_default(order) or "market_cap").lower()
    sector = coerce_query_default(sector)

    if order == "market_cap":
        candidates, total_f5_count = select_market_cap_candidates(db, max(200, n), sector)
    else:
        profile_cfg = RISK_PROFILES["neutro"]
        candidates, total_f5_count = select_candidates(db, profile_cfg, 636, sector)
    ...
    splits = get_split_dates()
    series = fetch_price_series(db, tickers, splits["calibration_start"], splits["calibration_end"])
    close_map = closes_from_series(series)
    min_len = min(len(close_map[ticker]) for ticker in tickers if ticker in close_map)
    returns = {
        ticker: daily_returns(close_map[ticker][-min_len:])
        for ticker in tickers
...
def get_ticker_return_diagnostics(
    ...
    if not row:
        raise HTTPException(status_code=404, detail="Ticker no encontrado en el universo de acciones.")
    meta = dict(row)

    splits = get_split_dates()
    series = fetch_price_series(db, [ticker], splits["calibration_start"], splits["calibration_end"])
    close_map = closes_from_series(series)
    closes = close_map.get(ticker, [])
    if len(closes) < 252:
        raise HTTPException(status_code=404, detail="Historia insuficiente para diagnostico de retornos.")
    returns = np.array(daily_returns(closes), dtype=float)
...
# ============================================================
# Recomendador automatico: top 3 portafolios por perfil
# ============================================================
...
def _run_single_optimization(
    ...
    prelim_candidates, total_f5_count = select_candidates(
        db=db, profile_cfg=profile_cfg, candidate_pool_size=636, sector_filter=sector,
    )
    pool_size = resolve_candidate_pool_size(profile_cfg, None, total_f5_count)
    if methodology_id == "equiponderado":
        candidates = sorted(prelim_candidates, key=lambda c: float(c.get("market_cap") or 0), reverse=True)[:pool_size]
    else:
        candidates = prelim_candidates[:pool_size]
    if not candidates:
        return None
    ...
    # Optimizacion
    optimized_weights = _run_primary_optimizer(
        methodology_id, valid_tickers, returns_matrix, ann_returns,
    ...
    return {
        "methodology_id": methodology_id,
        "methodology_label": RECOMMEND_LABELS.get(methodology_id, methodology_id),
        "profile": profile_key,
        "profile_label": profile_cfg["label"],
        "alpha_p": alpha_p,
        "portfolio": portfolio_rows(selected_tickers, selected_weights, metadata),
...
def recommend_portfolios(
    ...
    """Recomendador automatico: evalua metodologias y retorna los 3 mejores portafolios por Score P4."""
    if req.profile not in RISK_PROFILES:
        raise HTTPException(status_code=400, detail=f"Perfil no reconocido: {req.profile}")

    profile_cfg = RISK_PROFILES[req.profile]
    results = []
