...
from ..services.universe_f5 import f5_base_where_sql, get_universe_definition
...
@router.get("/api/universe/definition")
def get_universe_definition_endpoint():
    """Retorna la definicion completa del universo de acciones: filtros, criterios y supuestos."""
    return JSONResponse(
        content=get_universe_definition(),
...
def get_stock(ticker: str, db: sqlite3.Connection = Depends(get_db)):
    ...
    if row is None:
        raise HTTPException(status_code=404, detail=f"Ticker '{ticker}' no encontrado en el universo de acciones.")
