      ...
      <div class="pf-card">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input id="pf-capital" class="form-input" type="number" min="1000" step="1000" value="1000" placeholder="1000">
          </div>
          <div class="form-group">
...
async function submitPortfolioForm() {
  const capital    = parseFloat(document.getElementById("pf-capital").value) || 1000;
...
function renderSimulation() {
  ...
  const capital  = prefill ? 1000 : 1000;
      ...
      <div class="pf-card">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input id="sim-years" class="form-input" type="number" min="1" max="10" value="5">
          </div>
          <div class="form-group">
...
async function submitSimulation() {
  const capital  = parseFloat(document.getElementById("sim-capital")?.value)  || 1000;
  ...
  showContent(`
    <div class="welcome">
      <div class="welcome-panel">
        <div class="welcome-kicker">Universo de acciones</div>
...
function setNavState(view) {
  ...
  if (view === "home") {
    ...
    btnSector.style.display = "none";
    sep.style.display = "none";
    current.textContent = PF.currentModule === "portafolios" ? "Sistema recomendador FinPUC" : "Selecciona un sector del universo de acciones";
  } else if (view === "sector") {
    btnHome.style.display = "flex";
    btnSector.style.display = "none";
    sep.style.display = "none";
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <select id="pf-candidate-pool" class="form-select">
              ...
              <option value="636">universo de acciones completo</option>
            </select>
          </div>
          <div class="form-group">
  ...
  document.getElementById("pf-result").innerHTML = `
    ...
    <div class="detail-grid">
      <div class="pf-card">
        ...
        <div class="data-list">
          <div><span>Universo</span><strong>${esc(universe.name || "Universo de acciones")}</strong></div>
          <div><span>Universo disponible</span><strong>${universe.total_f5_count || "—"} acciones</strong></div>
...
async function simulateBaseCaseComparison(targetId = "benchmark-comparison") {
  ...
  const payload = {
    initial_capital: defaults.initial_capital ?? 1000,
  ...
  try {
    ...
    target.innerHTML = `
      <div class="pf-card" style="margin-top:14px">
        ...
        <div class="action-row" style="margin-top:12px">
          <button class="btn-secondary" type="button" onclick="selectStock('${esc(t)}')">Abrir ficha</button>
        </div>
...
function renderSimulation() {
  const capital = PF.lastInputs.capital || 1000;
  ...
  document.getElementById("content").innerHTML = `
    <div class="pf-section">
      ...
      <details class="details-card" open>
        <summary>Universo de acciones y filtros operativos</summary>
...
const PF_NAV_ITEMS = [
  { id: "recommend", label: "Recomendacion" },
...
function basePortfolioDefaults() {
  ...
  return {
    initial_capital: PF.formState.initial_capital || 1000,
...
function setNavState(view) {
  ...
  if (view === "home") {
    ...
    btnSector.style.display = "none";
    sep.style.display = "none";
    current.textContent = "Selecciona un sector del universo de acciones";
  } else if (view === "sector") {
    btnHome.style.display = "flex";
    btnSector.style.display = "none";
    sep.style.display = "none";
...
async function renderUniverseHome() {
  showContent('<div class="welcome"><div class="welcome-panel"><p>Cargando universo de acciones...</p></div></div>');
  try {
    const def = await apiFetch("/api/universe/definition");
    const stages = (def.filters?.stages || []).map(s => `
      <tr>
        <td><strong>${esc(s.id)}</strong></td>
        <td>${esc(s.name)}</td>
        <td style="font-size:0.85em;color:var(--fg-muted)">${esc(s.criterion)}</td>
        <td style="text-align:right">${s.tickers}</td>
        <td style="text-align:right">${s.retention_pct}%</td>
      </tr>
    `).join("");

    const criteria = def.current_criteria || {};
    const items = (def.assumptions?.items || []).map(a => `
      <div class="data-list-item">
        <span>${esc(a.label)}</span>
        <strong style="font-weight:400;text-align:left;max-width:520px">${esc(a.detail)}</strong>
      </div>
    `).join("");

    showContent(`
      <div class="pf-section" style="max-width:960px;margin:0 auto">
        <div class="hero-card">
          <div class="hero-eyebrow">Universo de acciones</div>
          <h2 class="hero-title">Definicion y supuestos del universo operativo</h2>
          <p class="hero-copy">${esc(def.description || "")} Los criterios y supuestos a continuacion estan basados en la Seccion 2.5 y el Anexo N2 del informe academico.</p>
        </div>

        <div class="pf-card">
          <div class="pf-section-title">Cascada de filtrado</div>
          <p style="margin-bottom:12px;color:var(--fg-muted)">Fuente: ${esc(def.filters?.source || "Anexo N2")}. Partiendo de 1.406 tickers crudos, se aplican filtros secuenciales hasta obtener un universo de 601 acciones estables e invertibles.</p>
            ...
            <table class="pf-table">
              <thead>
                <tr>
                  ...
                  <th style="text-align:right">Retencion</th>
                </tr>
              </thead>
              <tbody>${stages}</tbody>
            </table>
        ...
        <div class="pf-card">
          <div class="pf-section-title">Criterios operativos actuales</div>
          <div class="data-list" style="grid-template-columns:1fr 1fr">
            <div><span>Historia minima</span><strong>${criteria.min_history_years} anos (${criteria.min_history_rows} filas)</strong></div>
            <div><span>Precio minimo</span><strong>$${criteria.min_price_usd} USD</strong></div>
            <div><span>Market cap minimo</span><strong>$${(criteria.min_market_cap_usd / 1e9).toFixed(0)}B USD</strong></div>
            <div><span>Volatilidad anual</span><strong>${criteria.volatility_range_pct[0]}% - ${criteria.volatility_range_pct[1]}%</strong></div>
            <div><span>Excluye sector Unknown</span><strong>Si</strong></div>
            <div><span>Excluye Shell Companies</span><strong>Si</strong></div>
          </div>
        </div>

        <div class="pf-card">
          <div class="pf-section-title">Supuestos del universo operativo</div>
          <p style="margin-bottom:12px;color:var(--fg-muted)">Los siguientes supuestos estan documentados en la Seccion 2.4, Seccion 2.5, Seccion 3 y Anexo N2 del informe.</p>
          <div class="data-list" style="grid-template-columns:1fr">${items}</div>
        </div>

        <div class="pf-card">
          <div class="pf-section-title">Sectores disponibles</div>
          <div class="methodology-chip-row">
            ${(def.sectors || []).map(s => `<span class="methodology-chip" style="cursor:pointer" onclick="viewSector('${esc(s)}')">${esc(s)}</span>`).join("")}
          </div>
        </div>
      </div>
    `);
  } catch (error) {
    showContent('<div class="welcome"><div class="callout callout-danger">No se pudo cargar la definicion del universo: ' + esc(error.message) + '</div></div>');
  }
}
...
async function switchModule(mod) {
  ...
  if (mod === "portafolios") {
    tabFinpuc.classList.add("active");
    portNav.style.display = "block";
    document.getElementById("content").innerHTML = `<div class="results-empty">Cargando sistema FinPUC...</div>`;
    try {
      await ensurePortfolioBootstrap();
      if (!PF.activeNav || !PF_NAV_ITEMS.find(item => item.id === PF.activeNav)) {
        PF.activeNav = "recommend";
      }
      await pfNavGo(PF.activeNav);
    } catch (error) {
      document.getElementById("content").innerHTML = `<div class="callout callout-danger">No fue posible cargar el catalogo: ${esc(error.message)}</div>`;
    }
    return;
  }
...
function setMethodology(methodologyId) {
...
function renderRecommend() {
  ...
  const defaultProfile = PF.formState.profile || "neutro";

  document.getElementById("content").innerHTML = `
    <div class="pf-section">
      <div class="hero-card">
        <div class="hero-eyebrow">Recomendador FinPUC</div>
        <h2 class="hero-title">Tus 3 mejores portafolios</h2>
        <p class="hero-copy">Selecciona tu perfil de riesgo y FinPUC evaluara automaticamente las metodologias de la Entrega 2 para recomendarte los 3 portafolios con mejor relacion riesgo-beneficio, segun el Score P4.</p>
      </div>

      <div class="pf-card">
        <div class="pf-section-title">Perfil de riesgo</div>
        <div class="profile-selector">
          ${profiles.map(p => `
            <label class="profile-option ${defaultProfile === p.id ? "selected" : ""}" onclick="selectRecommendProfile('${esc(p.id)}')">
              <input type="radio" name="rec-profile" value="${esc(p.id)}" ${defaultProfile === p.id ? "checked" : ""}>
              <strong>${esc(p.label)}</strong>
              <span>Tolera hasta ${p.alpha_pct}% de perdida</span>
            </label>
          `).join("")}
        </div>
      </div>
      ...
      <div class="pf-card">
        <div class="pf-section-title">Capital y universo</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
          <div class="field-card">
            <label>Capital inicial (USD)</label>
            <input type="number" id="rec-capital" value="1000" min="500" step="500">
            <div class="field-help">C0 = $1,000 USD segun Entrega 2.</div>
          </div>
          <div class="field-card">
      ...
      <div class="pf-card">
        <div class="action-row">
          <button class="btn-primary" type="button" id="rec-submit-btn" onclick="submitRecommendation()" style="font-size:1.1em;padding:12px 32px">
            Obtener recomendaciones
          </button>
          <span id="rec-status" class="subtle-status"></span>
        </div>
      </div>
...
function selectRecommendProfile(profileId) {
  PF.formState.profile = profileId;
  document.querySelectorAll(".profile-option").forEach(el => el.classList.remove("selected"));
  const target = document.querySelector(`.profile-option input[value="${profileId}"]`)?.parentElement;
...
async function submitRecommendation() {
  ...
  if (btn) btn.disabled = true;
  if (status) status.textContent = "Evaluando metodologias...";
  ...
  try {
    ...
    if (container) container.innerHTML = `<div class="callout callout-danger">${esc(error.message)}</div>`;
  } finally {
    if (btn) btn.disabled = false;
  }
}
...
function renderRecommendationCards(result) {
  ...
  container.innerHTML = `
    <div class="pf-card">
      <div class="pf-section-title">Como se ordenan las recomendaciones</div>
      <p style="margin:0 0 4px">
        <strong>${esc(result.ranked_by || "Score P4")}:</strong> 
        ${esc(result.score_p4_formula || "Score = Riqueza + Utilidad − C0 × TasaRetiro")}
      </p>
      <p style="font-size:0.85em;color:var(--fg-muted);margin:0">
    ...
    ${recs.map(r => {
      ...
      const capNum = parseFloat((result.supuestos?.simulacion?.find(x => x.label.startsWith("C0"))?.value || "$1,000").replace(/[^0-9.]/g, "")) || 1000;
      const topWDRate = (recs.find(x => x.recommended)?.simulation?.withdrawal_rate) || 0;

      return `
      <div class="pf-card" style="margin-top:14px;${isRec ? 'border-left:4px solid var(--amber);' : 'opacity:0.85;'}">
        <div style="display:flex;align-items:flex-start;gap:14px;margin-bottom:14px">
          <span style="font-size:1.5em;font-weight:700;color:${isRec ? 'var(--amber)' : '#888'};line-height:1">
            #${r.rank}
          </span>
          <div style="flex:1;min-width:0">
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
              <span style="font-size:1.05em;font-weight:700">${esc(r.methodology_label)}</span>
              ${isRec ? '<span style="display:inline-block;background:var(--amber);color:#fff;font-size:0.68em;padding:1px 8px;border-radius:3px;font-weight:700">RECOMENDADO</span>' : ''}
            </div>
            <div style="font-size:0.82em;color:var(--fg-muted);margin-top:3px">
              Score P4: <strong>${fmtUSD(s.score_p4)}</strong>
              · ${isRec
                ? `Riqueza ${fmtUSD(s.final_capital_mean)} + Utilidad ${fmtUSD(s.total_commissions_mean)} − ${capNum} × ${(s.withdrawal_rate/100).toFixed(2)}`
                : `No alcanzo el top 3 — ${s.withdrawal_rate > topWDRate ? `mayor retiro (${s.withdrawal_rate}% vs ${topWDRate}%)` : `menor Score (${fmtUSD(s.score_p4)})`}`
              }
            </div>
          </div>
        </div>
        ...
        <div class="summary-grid">
          <div class="summary-card"><div class="summary-label">Retorno anual</div><div class="summary-value" style="font-size:1.1em">${m.expected_return_pct > 0 ? "+" : ""}${m.expected_return_pct}%</div></div>
          <div class="summary-card"><div class="summary-label">Volatilidad</div><div class="summary-value" style="font-size:1.1em">${m.volatility_pct}%</div></div>
          <div class="summary-card"><div class="summary-label">Sharpe</div><div class="summary-value" style="font-size:1.1em">${m.sharpe_ratio}</div></div>
          <div class="summary-card"><div class="summary-label">CVaR 95%</div><div class="summary-value" style="font-size:1.1em">${m.cvar_pct}%</div></div>
        </div>

        <div class="summary-grid">
          <div class="summary-card"><div class="summary-label">Riqueza final</div><div class="summary-value" style="font-size:1.1em">${fmtUSD(s.final_capital_mean)}</div></div>
          <div class="summary-card"><div class="summary-label">Tasa de retiro</div><div class="summary-value" style="font-size:1.1em">${s.withdrawal_rate}%</div></div>
          <div class="summary-card"><div class="summary-label">Utilidad FinPUC</div><div class="summary-value" style="font-size:1.1em">${fmtUSD(s.total_commissions_mean)}</div></div>
          <div class="summary-card"><div class="summary-label">P2 aceptacion</div><div class="summary-value" style="font-size:1.1em">${s.p2_acceptance_prob_pct}%</div></div>
        </div>

        <div class="scenario-grid" style="margin-bottom:8px">
          <div class="scenario-card favorable">
            <div class="scenario-label">Favorable +0.5σ</div>
            <div class="scenario-ret">${r.scenarios?.favorable?.annual_return_pct > 0 ? "+" : ""}${r.scenarios?.favorable?.annual_return_pct || "—"}% anual</div>
            <div class="scenario-cap">Capital año 5: <strong>${fmtUSD(r.scenarios?.favorable?.capital_by_year?.["5"])}</strong></div>
          </div>
          <div class="scenario-card neutro">
            <div class="scenario-label">Neutro</div>
            <div class="scenario-ret">${r.scenarios?.neutro?.annual_return_pct > 0 ? "+" : ""}${r.scenarios?.neutro?.annual_return_pct || "—"}% anual</div>
            <div class="scenario-cap">Capital año 5: <strong>${fmtUSD(r.scenarios?.neutro?.capital_by_year?.["5"])}</strong></div>
          </div>
          <div class="scenario-card desfavorable">
            <div class="scenario-label">Desfavorable −0.5σ</div>
            <div class="scenario-ret">${r.scenarios?.desfavorable?.annual_return_pct > 0 ? "+" : ""}${r.scenarios?.desfavorable?.annual_return_pct || "—"}% anual</div>
            <div class="scenario-cap">Capital año 5: <strong>${fmtUSD(r.scenarios?.desfavorable?.capital_by_year?.["5"])}</strong></div>
          </div>
        </div>
        ...
        <details>
          ...
          <table class="pf-table" style="margin-top:8px">
            ...
            <tbody>
              ${(r.portfolio || []).slice(0, 12).map(p => `
                <tr>
                  <td><strong>${esc(p.ticker)}</strong></td>
                  <td>${esc(p.short_name || "—")}</td>
                  <td>${esc(p.sector || "—")}</td>
                  <td style="text-align:right">${(p.weight * 100).toFixed(1)}%</td>
                  <td style="text-align:right">${p.cagr_pct != null ? p.cagr_pct + "%" : "—"}</td>
                  <td style="text-align:right">${p.volatility_pct != null ? p.volatility_pct + "%" : "—"}</td>
                </tr>
              `).join("")}
            </tbody>
    ...
    <div class="pf-section">
      ...
      <div class="pf-card">
        ...
        <div class="timeline">
          ...
          <div class="timeline-step">
            <strong>2. Universo operativo</strong>
            <p>Se selecciona un sub-universo compatible con el perfil y los filtros en cascada. Basado en Seccion 2.5, Tabla 2.4 y Tabla 0.10.</p>
          </div>
          <div class="timeline-step">
        ...
        ${methods.map(method => {
          ...
          <div class="method-card ${cardClass}" onclick="setMethodology('${esc(method.id)}')">
            ...
            <div class="action-row" style="margin-top:14px">
              <button class="btn-secondary" type="button" onclick="event.stopPropagation(); ${isWip ? "pfNavGo('views')" : `runMethodologyFromCatalog('${esc(method.id)}')`}">${isWip ? "Ver avance (beta)" : "Ejecutar ahora"}</button>
            </div>
          </div>`;
        }).join("")}
      </div>
...
async function runMethodologyFromCatalog(methodologyId) {
  setMethodology(methodologyId);
  PF.activeNav = "results";
  setPortfolioNavActive("results");
  document.getElementById("content").innerHTML = `<div class="pf-section"><div class="results-empty">Ejecutando ${esc(getCatalogMethod()?.label || methodologyId)}...</div></div>`;
  try {
    ...
    document.getElementById("content").innerHTML = `<div class="pf-section"><div class="callout callout-danger">No fue posible ejecutar: ${esc(error.message)}<br><button class="btn-secondary" style="margin-top:12px" onclick="pfNavGo('methodologies')">Volver al catalogo</button></div></div>`;
  }
}
...
function buildOptimizationPayload() {
  ...
  for (const definition of method.parameters || []) {
    if (definition.input_type !== "fixed") {
      parameterValues[definition.key] = PF.formState[definition.key];
    }
  }
  return {
    initial_capital: parseFloat(PF.formState.initial_capital || 1000),
      ...
      <div class="detail-grid">
        <div class="pf-card">
          ...
          <div class="data-list">
            ...
            <div><span>Universo de acciones total</span><strong>${universe.total_f5_count || "—"}</strong></div>
    ...
    contentEl.innerHTML = `
      <div class="pf-section">
        <div class="results-empty">
          ...
          <div class="action-row" style="margin-top:14px">
            <button class="btn-primary" type="button" onclick="pfNavGo('recommend')">Ir a recomendacion</button>
          </div>
        </div>
      </div>
    `;
...
function renderSimulation() {
  ...
  const capital = defaults.initial_capital || 1000;
...
async function submitSimulation() {
  const payload = {
    initial_capital: parseFloat(document.getElementById("sim-capital")?.value) || 1000,
    ...
    n_simulations: parseInt(document.getElementById("sim-n")?.value, 10) || 5000,
...
function renderReferencesView() {
  ...
  document.getElementById("content").innerHTML = `
    <div class="pf-section">
      ...
      <div class="pf-card">
        <div class="pf-section-title">Distribuciones de retornos</div>
...
async function pfNavGo(section) {
  ...
  if (section === "recommend") renderRecommend();
  ...
  document.getElementById("content").innerHTML = `
    <div class="pf-section">
      ...
      <div class="pf-card">
        ...
        <p style="font-size:13px;color:var(--text-muted);margin-bottom:16px;">
          Ingresa el ticker (del universo de acciones) y el retorno anualizado que esperas para esa accion.
